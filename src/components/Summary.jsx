...
data[3].icon = img4
 const tabItem = data.map(data => {
  return <Tab
    ...
    return(
        <>
         <section aria-labelledby="sum-head" className={`xl:${width} flex  xl:pb-5   xl:${height} justify-center xl:items-center ${mobileWidth} ${mobileHeight} md:h-full`}>
          <div className={`flex flex-col   w-[95%] xl:h-[95%]  justify-between md:h-[90%] md:mt-6`} >
           <h1 id="sum-head" className="text-2xl text-darkGrayBlue font-bold">Summary</h1>
 ...
 function Tab({category,icon,score,color,backgroundColor,alt}) {
    const text = {
...
function Button() {
    ...
    return <button className={`rounded-full bg-darkGrayBlue text-white h-[60px] mt-5 ${isClicked ? 'transition-all duration-700 ease-linear bg-gradient-to-b from-slateBlue to-lightRoyalBlue animate-beat': null}`} onClick={checkClick}>Continue</button>
}